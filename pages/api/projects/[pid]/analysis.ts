...
const promptBuilder = (type: string, name: string) => {
  ...
  switch (type) {
    case 'swot':
      ...
      Based on this outline, give me a professional and detailed SWOT analysis of ${name}.`;
      break;
...
async function getChatGPT(prompt: string) {
  const api = new ChatGPTAPI({ sessionToken: SESSION_TOKEN as string }); //process.env.SESSION_TOKEN
